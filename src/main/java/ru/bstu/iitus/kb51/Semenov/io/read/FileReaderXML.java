 ...
 */
public class FileReaderXML implements StrategyReader {
    private File file;
    ...
    void initReg() {
        ...
        patterns.add(Pattern.compile(template));
        template = "<include name=\".*\" age=\"(.*)\">";
        patterns.add(Pattern.compile(template));
        template = "<property name=\"arg\">(.*)";
        for (ParametersType p : ParametersType.values()) {
            if(!p.equals(ParametersType.TYPE)&&!p.equals(ParametersType.NAME)&&!p.equals(ParametersType.AGE)) {
    ...
    public FileReaderXML(String path) {
        initStrategy(path);
    }
    ...
    public Object readParams(ParametersType type) throws FatalInvalidInput {
       ...
       throw new FatalInvalidInput("FileReaderXML.readParams");
    }
